# JMH version: 1.21
# VM version: JDK 1.8.0_412, OpenJDK 64-Bit Server VM, 25.412-b08
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.412-8/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Throughput, ops/time
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.createUser

# Run progress: 0.00% complete, ETA 00:00:24
# Fork: 1 of 1
# Warmup Iteration   1: 0.917 ops/ms
Iteration   1: 2.187 ops/ms


Result "org.apache.dubbo.benchmark.ClientSimple.createUser":
  2.187 ops/ms


# JMH version: 1.21
# VM version: JDK 1.8.0_412, OpenJDK 64-Bit Server VM, 25.412-b08
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.412-8/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Throughput, ops/time
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.existUser

# Run progress: 8.33% complete, ETA 00:01:18
# Fork: 1 of 1
# Warmup Iteration   1: 3.567 ops/ms
Iteration   1: 6.568 ops/ms


Result "org.apache.dubbo.benchmark.ClientSimple.existUser":
  6.568 ops/ms


# JMH version: 1.21
# VM version: JDK 1.8.0_412, OpenJDK 64-Bit Server VM, 25.412-b08
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.412-8/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Throughput, ops/time
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.getUser

# Run progress: 16.67% complete, ETA 00:01:10
# Fork: 1 of 1
# Warmup Iteration   1: 2.243 ops/ms
Iteration   1: 4.028 ops/ms


Result "org.apache.dubbo.benchmark.ClientSimple.getUser":
  4.028 ops/ms


# JMH version: 1.21
# VM version: JDK 1.8.0_412, OpenJDK 64-Bit Server VM, 25.412-b08
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.412-8/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Throughput, ops/time
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.listUser

# Run progress: 25.00% complete, ETA 00:01:03
# Fork: 1 of 1
# Warmup Iteration   1: 2.216 ops/ms
Iteration   1: 3.150 ops/ms


Result "org.apache.dubbo.benchmark.ClientSimple.listUser":
  3.150 ops/ms


# JMH version: 1.21
# VM version: JDK 1.8.0_412, OpenJDK 64-Bit Server VM, 25.412-b08
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.412-8/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Average time, time/op
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.createUser

# Run progress: 33.33% complete, ETA 00:00:56
# Fork: 1 of 1
# Warmup Iteration   1: 11.490 ±(99.9%) 0.291 ms/op
Iteration   1: 6.720 ±(99.9%) 0.049 ms/op


Result "org.apache.dubbo.benchmark.ClientSimple.createUser":
  6.720 ms/op


# JMH version: 1.21
# VM version: JDK 1.8.0_412, OpenJDK 64-Bit Server VM, 25.412-b08
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.412-8/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Average time, time/op
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.existUser

# Run progress: 41.67% complete, ETA 00:00:49
# Fork: 1 of 1
# Warmup Iteration   1: 5.950 ±(99.9%) 0.094 ms/op
Iteration   1: 3.125 ±(99.9%) 0.012 ms/op


Result "org.apache.dubbo.benchmark.ClientSimple.existUser":
  3.125 ms/op


# JMH version: 1.21
# VM version: JDK 1.8.0_412, OpenJDK 64-Bit Server VM, 25.412-b08
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.412-8/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Average time, time/op
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.getUser

# Run progress: 50.00% complete, ETA 00:00:41
# Fork: 1 of 1
# Warmup Iteration   1: 10.599 ±(99.9%) 0.161 ms/op
Iteration   1: 7.256 ±(99.9%) 0.055 ms/op


Result "org.apache.dubbo.benchmark.ClientSimple.getUser":
  7.256 ms/op


# JMH version: 1.21
# VM version: JDK 1.8.0_412, OpenJDK 64-Bit Server VM, 25.412-b08
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.412-8/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Average time, time/op
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.listUser

# Run progress: 58.33% complete, ETA 00:00:34
# Fork: 1 of 1
# Warmup Iteration   1: 12.731 ±(99.9%) 0.292 ms/op
Iteration   1: 9.863 ±(99.9%) 0.086 ms/op


Result "org.apache.dubbo.benchmark.ClientSimple.listUser":
  9.863 ms/op


# JMH version: 1.21
# VM version: JDK 1.8.0_412, OpenJDK 64-Bit Server VM, 25.412-b08
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.412-8/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Sampling time
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.createUser

# Run progress: 66.67% complete, ETA 00:00:27
# Fork: 1 of 1
# Warmup Iteration   1: 11.113 ±(99.9%) 0.320 ms/op
Iteration   1: 6.889 ±(99.9%) 0.129 ms/op
                 createUser·p0.00:   2.146 ms/op
                 createUser·p0.50:   6.472 ms/op
                 createUser·p0.90:   8.716 ms/op
                 createUser·p0.95:   11.452 ms/op
                 createUser·p0.99:   19.616 ms/op
                 createUser·p0.999:  27.263 ms/op
                 createUser·p0.9999: 27.329 ms/op
                 createUser·p1.00:   27.329 ms/op



Result "org.apache.dubbo.benchmark.ClientSimple.createUser":
  N = 4637
  mean =      6.889 ±(99.9%) 0.129 ms/op

  Histogram, ms/op:
    [ 0.000,  2.500) = 4 
    [ 2.500,  5.000) = 626 
    [ 5.000,  7.500) = 3213 
    [ 7.500, 10.000) = 461 
    [10.000, 12.500) = 155 
    [12.500, 15.000) = 96 
    [15.000, 17.500) = 18 
    [17.500, 20.000) = 19 
    [20.000, 22.500) = 6 
    [22.500, 25.000) = 24 
    [25.000, 27.500) = 15 

  Percentiles, ms/op:
      p(0.0000) =      2.146 ms/op
     p(50.0000) =      6.472 ms/op
     p(90.0000) =      8.716 ms/op
     p(95.0000) =     11.452 ms/op
     p(99.0000) =     19.616 ms/op
     p(99.9000) =     27.263 ms/op
     p(99.9900) =     27.329 ms/op
     p(99.9990) =     27.329 ms/op
     p(99.9999) =     27.329 ms/op
    p(100.0000) =     27.329 ms/op


# JMH version: 1.21
# VM version: JDK 1.8.0_412, OpenJDK 64-Bit Server VM, 25.412-b08
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.412-8/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Sampling time
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.existUser

# Run progress: 75.00% complete, ETA 00:00:20
# Fork: 1 of 1
# Warmup Iteration   1: 5.702 ±(99.9%) 0.164 ms/op
Iteration   1: 3.307 ±(99.9%) 0.069 ms/op
                 existUser·p0.00:   0.686 ms/op
                 existUser·p0.50:   3.035 ms/op
                 existUser·p0.90:   4.063 ms/op
                 existUser·p0.95:   4.579 ms/op
                 existUser·p0.99:   10.306 ms/op
                 existUser·p0.999:  34.185 ms/op
                 existUser·p0.9999: 35.586 ms/op
                 existUser·p1.00:   35.586 ms/op



Result "org.apache.dubbo.benchmark.ClientSimple.existUser":
  N = 9685
  mean =      3.307 ±(99.9%) 0.069 ms/op

  Histogram, ms/op:
    [ 0.000,  2.500) = 1688 
    [ 2.500,  5.000) = 7680 
    [ 5.000,  7.500) = 184 
    [ 7.500, 10.000) = 36 
    [10.000, 12.500) = 25 
    [12.500, 15.000) = 39 
    [15.000, 17.500) = 1 
    [17.500, 20.000) = 0 
    [20.000, 22.500) = 0 
    [22.500, 25.000) = 0 
    [25.000, 27.500) = 0 
    [27.500, 30.000) = 0 
    [30.000, 32.500) = 7 
    [32.500, 35.000) = 22 
    [35.000, 37.500) = 3 

  Percentiles, ms/op:
      p(0.0000) =      0.686 ms/op
     p(50.0000) =      3.035 ms/op
     p(90.0000) =      4.063 ms/op
     p(95.0000) =      4.579 ms/op
     p(99.0000) =     10.306 ms/op
     p(99.9000) =     34.185 ms/op
     p(99.9900) =     35.586 ms/op
     p(99.9990) =     35.586 ms/op
     p(99.9999) =     35.586 ms/op
    p(100.0000) =     35.586 ms/op


# JMH version: 1.21
# VM version: JDK 1.8.0_412, OpenJDK 64-Bit Server VM, 25.412-b08
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.412-8/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Sampling time
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.getUser

# Run progress: 83.33% complete, ETA 00:00:14
# Fork: 1 of 1
# Warmup Iteration   1: 8.833 ±(99.9%) 0.235 ms/op
Iteration   1: 6.724 ±(99.9%) 0.187 ms/op
                 getUser·p0.00:   1.620 ms/op
                 getUser·p0.50:   6.152 ms/op
                 getUser·p0.90:   8.487 ms/op
                 getUser·p0.95:   9.684 ms/op
                 getUser·p0.99:   29.028 ms/op
                 getUser·p0.999:  46.300 ms/op
                 getUser·p0.9999: 49.086 ms/op
                 getUser·p1.00:   49.086 ms/op



Result "org.apache.dubbo.benchmark.ClientSimple.getUser":
  N = 4758
  mean =      6.724 ±(99.9%) 0.187 ms/op

  Histogram, ms/op:
    [ 0.000,  5.000) = 1083 
    [ 5.000, 10.000) = 3455 
    [10.000, 15.000) = 123 
    [15.000, 20.000) = 29 
    [20.000, 25.000) = 5 
    [25.000, 30.000) = 23 
    [30.000, 35.000) = 15 
    [35.000, 40.000) = 6 
    [40.000, 45.000) = 13 

  Percentiles, ms/op:
      p(0.0000) =      1.620 ms/op
     p(50.0000) =      6.152 ms/op
     p(90.0000) =      8.487 ms/op
     p(95.0000) =      9.684 ms/op
     p(99.0000) =     29.028 ms/op
     p(99.9000) =     46.300 ms/op
     p(99.9900) =     49.086 ms/op
     p(99.9990) =     49.086 ms/op
     p(99.9999) =     49.086 ms/op
    p(100.0000) =     49.086 ms/op


# JMH version: 1.21
# VM version: JDK 1.8.0_412, OpenJDK 64-Bit Server VM, 25.412-b08
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.412-8/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Sampling time
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.listUser

# Run progress: 91.67% complete, ETA 00:00:07
# Fork: 1 of 1
# Warmup Iteration   1: 11.797 ±(99.9%) 0.392 ms/op
Iteration   1: 9.998 ±(99.9%) 0.194 ms/op
                 listUser·p0.00:   2.793 ms/op
                 listUser·p0.50:   9.634 ms/op
                 listUser·p0.90:   12.698 ms/op
                 listUser·p0.95:   14.074 ms/op
                 listUser·p0.99:   26.707 ms/op
                 listUser·p0.999:  31.279 ms/op
                 listUser·p0.9999: 32.997 ms/op
                 listUser·p1.00:   32.997 ms/op



Result "org.apache.dubbo.benchmark.ClientSimple.listUser":
  N = 3223
  mean =      9.998 ±(99.9%) 0.194 ms/op

  Histogram, ms/op:
    [ 0.000,  2.500) = 0 
    [ 2.500,  5.000) = 63 
    [ 5.000,  7.500) = 442 
    [ 7.500, 10.000) = 1353 
    [10.000, 12.500) = 1001 
    [12.500, 15.000) = 231 
    [15.000, 17.500) = 36 
    [17.500, 20.000) = 27 
    [20.000, 22.500) = 5 
    [22.500, 25.000) = 23 
    [25.000, 27.500) = 21 
    [27.500, 30.000) = 16 
    [30.000, 32.500) = 4 
    [32.500, 35.000) = 1 
    [35.000, 37.500) = 0 

  Percentiles, ms/op:
      p(0.0000) =      2.793 ms/op
     p(50.0000) =      9.634 ms/op
     p(90.0000) =     12.698 ms/op
     p(95.0000) =     14.074 ms/op
     p(99.0000) =     26.707 ms/op
     p(99.9000) =     31.279 ms/op
     p(99.9900) =     32.997 ms/op
     p(99.9990) =     32.997 ms/op
     p(99.9999) =     32.997 ms/op
    p(100.0000) =     32.997 ms/op


# Run complete. Total time: 00:01:24

REMEMBER: The numbers below are just data. To gain reusable insights, you need to follow up on
why the numbers are the way they are. Use profilers (see -prof, -lprof), design factorial
experiments, perform baseline and negative tests that provide experimental control, make sure
the benchmarking environment is safe on JVM/OS/HW level, ask for reviews from the domain experts.
Do not assume the numbers tell you what you want them to tell.

Benchmark                                     Mode   Cnt   Score   Error   Units
ClientSimple.createUser                      thrpt         2.187          ops/ms
ClientSimple.existUser                       thrpt         6.568          ops/ms
ClientSimple.getUser                         thrpt         4.028          ops/ms
ClientSimple.listUser                        thrpt         3.150          ops/ms
ClientSimple.createUser                       avgt         6.720           ms/op
ClientSimple.existUser                        avgt         3.125           ms/op
ClientSimple.getUser                          avgt         7.256           ms/op
ClientSimple.listUser                         avgt         9.863           ms/op
ClientSimple.createUser                     sample  4637   6.889 ± 0.129   ms/op
ClientSimple.createUser:createUser·p0.00    sample         2.146           ms/op
ClientSimple.createUser:createUser·p0.50    sample         6.472           ms/op
ClientSimple.createUser:createUser·p0.90    sample         8.716           ms/op
ClientSimple.createUser:createUser·p0.95    sample        11.452           ms/op
ClientSimple.createUser:createUser·p0.99    sample        19.616           ms/op
ClientSimple.createUser:createUser·p0.999   sample        27.263           ms/op
ClientSimple.createUser:createUser·p0.9999  sample        27.329           ms/op
ClientSimple.createUser:createUser·p1.00    sample        27.329           ms/op
ClientSimple.existUser                      sample  9685   3.307 ± 0.069   ms/op
ClientSimple.existUser:existUser·p0.00      sample         0.686           ms/op
ClientSimple.existUser:existUser·p0.50      sample         3.035           ms/op
ClientSimple.existUser:existUser·p0.90      sample         4.063           ms/op
ClientSimple.existUser:existUser·p0.95      sample         4.579           ms/op
ClientSimple.existUser:existUser·p0.99      sample        10.306           ms/op
ClientSimple.existUser:existUser·p0.999     sample        34.185           ms/op
ClientSimple.existUser:existUser·p0.9999    sample        35.586           ms/op
ClientSimple.existUser:existUser·p1.00      sample        35.586           ms/op
ClientSimple.getUser                        sample  4758   6.724 ± 0.187   ms/op
ClientSimple.getUser:getUser·p0.00          sample         1.620           ms/op
ClientSimple.getUser:getUser·p0.50          sample         6.152           ms/op
ClientSimple.getUser:getUser·p0.90          sample         8.487           ms/op
ClientSimple.getUser:getUser·p0.95          sample         9.684           ms/op
ClientSimple.getUser:getUser·p0.99          sample        29.028           ms/op
ClientSimple.getUser:getUser·p0.999         sample        46.300           ms/op
ClientSimple.getUser:getUser·p0.9999        sample        49.086           ms/op
ClientSimple.getUser:getUser·p1.00          sample        49.086           ms/op
ClientSimple.listUser                       sample  3223   9.998 ± 0.194   ms/op
ClientSimple.listUser:listUser·p0.00        sample         2.793           ms/op
ClientSimple.listUser:listUser·p0.50        sample         9.634           ms/op
ClientSimple.listUser:listUser·p0.90        sample        12.698           ms/op
ClientSimple.listUser:listUser·p0.95        sample        14.074           ms/op
ClientSimple.listUser:listUser·p0.99        sample        26.707           ms/op
ClientSimple.listUser:listUser·p0.999       sample        31.279           ms/op
ClientSimple.listUser:listUser·p0.9999      sample        32.997           ms/op
ClientSimple.listUser:listUser·p1.00        sample        32.997           ms/op

Benchmark result is saved to 1720872354219.json
