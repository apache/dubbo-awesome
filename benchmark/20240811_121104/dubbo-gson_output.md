# JMH version: 1.21
# VM version: JDK 1.8.0_422, OpenJDK 64-Bit Server VM, 25.422-b05
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.422-5/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Throughput, ops/time
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.createUser

# Run progress: 0.00% complete, ETA 00:00:24
# Fork: 1 of 1
# Warmup Iteration   1: 0.585 ops/ms
Iteration   1: 1.814 ops/ms


Result "org.apache.dubbo.benchmark.ClientSimple.createUser":
  1.814 ops/ms


# JMH version: 1.21
# VM version: JDK 1.8.0_422, OpenJDK 64-Bit Server VM, 25.422-b05
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.422-5/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Throughput, ops/time
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.existUser

# Run progress: 8.33% complete, ETA 00:01:17
# Fork: 1 of 1
# Warmup Iteration   1: 3.207 ops/ms
Iteration   1: 6.649 ops/ms


Result "org.apache.dubbo.benchmark.ClientSimple.existUser":
  6.649 ops/ms


# JMH version: 1.21
# VM version: JDK 1.8.0_422, OpenJDK 64-Bit Server VM, 25.422-b05
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.422-5/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Throughput, ops/time
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.getUser

# Run progress: 16.67% complete, ETA 00:01:10
# Fork: 1 of 1
# Warmup Iteration   1: 1.999 ops/ms
Iteration   1: 3.598 ops/ms


Result "org.apache.dubbo.benchmark.ClientSimple.getUser":
  3.598 ops/ms


# JMH version: 1.21
# VM version: JDK 1.8.0_422, OpenJDK 64-Bit Server VM, 25.422-b05
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.422-5/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Throughput, ops/time
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.listUser

# Run progress: 25.00% complete, ETA 00:01:03
# Fork: 1 of 1
# Warmup Iteration   1: 1.825 ops/ms
Iteration   1: 2.621 ops/ms


Result "org.apache.dubbo.benchmark.ClientSimple.listUser":
  2.621 ops/ms


# JMH version: 1.21
# VM version: JDK 1.8.0_422, OpenJDK 64-Bit Server VM, 25.422-b05
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.422-5/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Average time, time/op
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.createUser

# Run progress: 33.33% complete, ETA 00:00:56
# Fork: 1 of 1
# Warmup Iteration   1: 13.306 ±(99.9%) 0.269 ms/op
Iteration   1: 7.248 ±(99.9%) 0.106 ms/op


Result "org.apache.dubbo.benchmark.ClientSimple.createUser":
  7.248 ms/op


# JMH version: 1.21
# VM version: JDK 1.8.0_422, OpenJDK 64-Bit Server VM, 25.422-b05
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.422-5/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Average time, time/op
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.existUser

# Run progress: 41.67% complete, ETA 00:00:49
# Fork: 1 of 1
# Warmup Iteration   1: 6.929 ±(99.9%) 0.111 ms/op
Iteration   1: 3.686 ±(99.9%) 0.039 ms/op


Result "org.apache.dubbo.benchmark.ClientSimple.existUser":
  3.686 ms/op


# JMH version: 1.21
# VM version: JDK 1.8.0_422, OpenJDK 64-Bit Server VM, 25.422-b05
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.422-5/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Average time, time/op
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.getUser

# Run progress: 50.00% complete, ETA 00:00:42
# Fork: 1 of 1
# Warmup Iteration   1: 11.193 ±(99.9%) 0.264 ms/op
Iteration   1: 7.176 ±(99.9%) 0.057 ms/op


Result "org.apache.dubbo.benchmark.ClientSimple.getUser":
  7.176 ms/op


# JMH version: 1.21
# VM version: JDK 1.8.0_422, OpenJDK 64-Bit Server VM, 25.422-b05
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.422-5/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Average time, time/op
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.listUser

# Run progress: 58.33% complete, ETA 00:00:35
# Fork: 1 of 1
# Warmup Iteration   1: 13.946 ±(99.9%) 0.297 ms/op
Iteration   1: 10.965 ±(99.9%) 0.097 ms/op


Result "org.apache.dubbo.benchmark.ClientSimple.listUser":
  10.965 ms/op


# JMH version: 1.21
# VM version: JDK 1.8.0_422, OpenJDK 64-Bit Server VM, 25.422-b05
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.422-5/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Sampling time
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.createUser

# Run progress: 66.67% complete, ETA 00:00:28
# Fork: 1 of 1
# Warmup Iteration   1: 10.955 ±(99.9%) 0.291 ms/op
Iteration   1: 7.986 ±(99.9%) 0.177 ms/op
                 createUser·p0.00:   3.346 ms/op
                 createUser·p0.50:   7.086 ms/op
                 createUser·p0.90:   11.944 ms/op
                 createUser·p0.95:   16.581 ms/op
                 createUser·p0.99:   21.266 ms/op
                 createUser·p0.999:  22.657 ms/op
                 createUser·p0.9999: 28.803 ms/op
                 createUser·p1.00:   28.803 ms/op



Result "org.apache.dubbo.benchmark.ClientSimple.createUser":
  N = 4081
  mean =      7.986 ±(99.9%) 0.177 ms/op

  Histogram, ms/op:
    [ 0.000,  2.500) = 0 
    [ 2.500,  5.000) = 277 
    [ 5.000,  7.500) = 2191 
    [ 7.500, 10.000) = 1038 
    [10.000, 12.500) = 197 
    [12.500, 15.000) = 145 
    [15.000, 17.500) = 62 
    [17.500, 20.000) = 53 
    [20.000, 22.500) = 114 
    [22.500, 25.000) = 2 
    [25.000, 27.500) = 1 

  Percentiles, ms/op:
      p(0.0000) =      3.346 ms/op
     p(50.0000) =      7.086 ms/op
     p(90.0000) =     11.944 ms/op
     p(95.0000) =     16.581 ms/op
     p(99.0000) =     21.266 ms/op
     p(99.9000) =     22.657 ms/op
     p(99.9900) =     28.803 ms/op
     p(99.9990) =     28.803 ms/op
     p(99.9999) =     28.803 ms/op
    p(100.0000) =     28.803 ms/op


# JMH version: 1.21
# VM version: JDK 1.8.0_422, OpenJDK 64-Bit Server VM, 25.422-b05
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.422-5/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Sampling time
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.existUser

# Run progress: 75.00% complete, ETA 00:00:21
# Fork: 1 of 1
# Warmup Iteration   1: 5.934 ±(99.9%) 0.138 ms/op
Iteration   1: 3.481 ±(99.9%) 0.049 ms/op
                 existUser·p0.00:   0.702 ms/op
                 existUser·p0.50:   3.203 ms/op
                 existUser·p0.90:   4.456 ms/op
                 existUser·p0.95:   5.005 ms/op
                 existUser·p0.99:   12.943 ms/op
                 existUser·p0.999:  14.074 ms/op
                 existUser·p0.9999: 14.270 ms/op
                 existUser·p1.00:   14.270 ms/op



Result "org.apache.dubbo.benchmark.ClientSimple.existUser":
  N = 9180
  mean =      3.481 ±(99.9%) 0.049 ms/op

  Histogram, ms/op:
    [ 0.000,  1.250) = 25 
    [ 1.250,  2.500) = 675 
    [ 2.500,  3.750) = 6279 
    [ 3.750,  5.000) = 1740 
    [ 5.000,  6.250) = 196 
    [ 6.250,  7.500) = 104 
    [ 7.500,  8.750) = 32 
    [ 8.750, 10.000) = 1 
    [10.000, 11.250) = 4 
    [11.250, 12.500) = 20 
    [12.500, 13.750) = 83 
    [13.750, 15.000) = 21 
    [15.000, 16.250) = 0 
    [16.250, 17.500) = 0 
    [17.500, 18.750) = 0 

  Percentiles, ms/op:
      p(0.0000) =      0.702 ms/op
     p(50.0000) =      3.203 ms/op
     p(90.0000) =      4.456 ms/op
     p(95.0000) =      5.005 ms/op
     p(99.0000) =     12.943 ms/op
     p(99.9000) =     14.074 ms/op
     p(99.9900) =     14.270 ms/op
     p(99.9990) =     14.270 ms/op
     p(99.9999) =     14.270 ms/op
    p(100.0000) =     14.270 ms/op


# JMH version: 1.21
# VM version: JDK 1.8.0_422, OpenJDK 64-Bit Server VM, 25.422-b05
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.422-5/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Sampling time
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.getUser

# Run progress: 83.33% complete, ETA 00:00:14
# Fork: 1 of 1
# Warmup Iteration   1: 10.388 ±(99.9%) 0.341 ms/op
Iteration   1: 7.078 ±(99.9%) 0.083 ms/op
                 getUser·p0.00:   1.980 ms/op
                 getUser·p0.50:   6.939 ms/op
                 getUser·p0.90:   8.913 ms/op
                 getUser·p0.95:   10.048 ms/op
                 getUser·p0.99:   12.913 ms/op
                 getUser·p0.999:  16.826 ms/op
                 getUser·p0.9999: 17.695 ms/op
                 getUser·p1.00:   17.695 ms/op



Result "org.apache.dubbo.benchmark.ClientSimple.getUser":
  N = 4513
  mean =      7.078 ±(99.9%) 0.083 ms/op

  Histogram, ms/op:
    [ 0.000,  1.250) = 0 
    [ 1.250,  2.500) = 6 
    [ 2.500,  3.750) = 31 
    [ 3.750,  5.000) = 285 
    [ 5.000,  6.250) = 985 
    [ 6.250,  7.500) = 1675 
    [ 7.500,  8.750) = 1028 
    [ 8.750, 10.000) = 275 
    [10.000, 11.250) = 129 
    [11.250, 12.500) = 40 
    [12.500, 13.750) = 26 
    [13.750, 15.000) = 8 
    [15.000, 16.250) = 17 
    [16.250, 17.500) = 6 
    [17.500, 18.750) = 2 

  Percentiles, ms/op:
      p(0.0000) =      1.980 ms/op
     p(50.0000) =      6.939 ms/op
     p(90.0000) =      8.913 ms/op
     p(95.0000) =     10.048 ms/op
     p(99.0000) =     12.913 ms/op
     p(99.9000) =     16.826 ms/op
     p(99.9900) =     17.695 ms/op
     p(99.9990) =     17.695 ms/op
     p(99.9999) =     17.695 ms/op
    p(100.0000) =     17.695 ms/op


# JMH version: 1.21
# VM version: JDK 1.8.0_422, OpenJDK 64-Bit Server VM, 25.422-b05
# VM invoker: /opt/hostedtoolcache/Java_Adopt_jdk/8.0.422-5/x64/jre/bin/java
# VM options: -Xmx1g -Xms1g -XX:MaxDirectMemorySize=1g -XX:+UseG1GC -Dserver.host=localhost -Dserver.port=8080 -Dbenchmark.output=dubbo-gson_output.md
# Warmup: 1 iterations, 1 s each
# Measurement: 1 iterations, 1 s each
# Timeout: 10 min per iteration
# Threads: 32 threads, will synchronize iterations
# Benchmark mode: Sampling time
# Benchmark: org.apache.dubbo.benchmark.ClientSimple.listUser

# Run progress: 91.67% complete, ETA 00:00:07
# Fork: 1 of 1
# Warmup Iteration   1: 12.111 ±(99.9%) 0.459 ms/op
Iteration   1: 10.771 ±(99.9%) 0.233 ms/op
                 listUser·p0.00:   3.117 ms/op
                 listUser·p0.50:   10.109 ms/op
                 listUser·p0.90:   13.774 ms/op
                 listUser·p0.95:   15.408 ms/op
                 listUser·p0.99:   37.168 ms/op
                 listUser·p0.999:  41.551 ms/op
                 listUser·p0.9999: 45.285 ms/op
                 listUser·p1.00:   45.285 ms/op



Result "org.apache.dubbo.benchmark.ClientSimple.listUser":
  N = 2992
  mean =     10.771 ±(99.9%) 0.233 ms/op

  Histogram, ms/op:
    [ 0.000,  5.000) = 21 
    [ 5.000, 10.000) = 1411 
    [10.000, 15.000) = 1397 
    [15.000, 20.000) = 117 
    [20.000, 25.000) = 14 
    [25.000, 30.000) = 0 
    [30.000, 35.000) = 0 
    [35.000, 40.000) = 25 
    [40.000, 45.000) = 6 

  Percentiles, ms/op:
      p(0.0000) =      3.117 ms/op
     p(50.0000) =     10.109 ms/op
     p(90.0000) =     13.774 ms/op
     p(95.0000) =     15.408 ms/op
     p(99.0000) =     37.168 ms/op
     p(99.9000) =     41.551 ms/op
     p(99.9900) =     45.285 ms/op
     p(99.9990) =     45.285 ms/op
     p(99.9999) =     45.285 ms/op
    p(100.0000) =     45.285 ms/op


# Run complete. Total time: 00:01:25

REMEMBER: The numbers below are just data. To gain reusable insights, you need to follow up on
why the numbers are the way they are. Use profilers (see -prof, -lprof), design factorial
experiments, perform baseline and negative tests that provide experimental control, make sure
the benchmarking environment is safe on JVM/OS/HW level, ask for reviews from the domain experts.
Do not assume the numbers tell you what you want them to tell.

Benchmark                                     Mode   Cnt   Score   Error   Units
ClientSimple.createUser                      thrpt         1.814          ops/ms
ClientSimple.existUser                       thrpt         6.649          ops/ms
ClientSimple.getUser                         thrpt         3.598          ops/ms
ClientSimple.listUser                        thrpt         2.621          ops/ms
ClientSimple.createUser                       avgt         7.248           ms/op
ClientSimple.existUser                        avgt         3.686           ms/op
ClientSimple.getUser                          avgt         7.176           ms/op
ClientSimple.listUser                         avgt        10.965           ms/op
ClientSimple.createUser                     sample  4081   7.986 ± 0.177   ms/op
ClientSimple.createUser:createUser·p0.00    sample         3.346           ms/op
ClientSimple.createUser:createUser·p0.50    sample         7.086           ms/op
ClientSimple.createUser:createUser·p0.90    sample        11.944           ms/op
ClientSimple.createUser:createUser·p0.95    sample        16.581           ms/op
ClientSimple.createUser:createUser·p0.99    sample        21.266           ms/op
ClientSimple.createUser:createUser·p0.999   sample        22.657           ms/op
ClientSimple.createUser:createUser·p0.9999  sample        28.803           ms/op
ClientSimple.createUser:createUser·p1.00    sample        28.803           ms/op
ClientSimple.existUser                      sample  9180   3.481 ± 0.049   ms/op
ClientSimple.existUser:existUser·p0.00      sample         0.702           ms/op
ClientSimple.existUser:existUser·p0.50      sample         3.203           ms/op
ClientSimple.existUser:existUser·p0.90      sample         4.456           ms/op
ClientSimple.existUser:existUser·p0.95      sample         5.005           ms/op
ClientSimple.existUser:existUser·p0.99      sample        12.943           ms/op
ClientSimple.existUser:existUser·p0.999     sample        14.074           ms/op
ClientSimple.existUser:existUser·p0.9999    sample        14.270           ms/op
ClientSimple.existUser:existUser·p1.00      sample        14.270           ms/op
ClientSimple.getUser                        sample  4513   7.078 ± 0.083   ms/op
ClientSimple.getUser:getUser·p0.00          sample         1.980           ms/op
ClientSimple.getUser:getUser·p0.50          sample         6.939           ms/op
ClientSimple.getUser:getUser·p0.90          sample         8.913           ms/op
ClientSimple.getUser:getUser·p0.95          sample        10.048           ms/op
ClientSimple.getUser:getUser·p0.99          sample        12.913           ms/op
ClientSimple.getUser:getUser·p0.999         sample        16.826           ms/op
ClientSimple.getUser:getUser·p0.9999        sample        17.695           ms/op
ClientSimple.getUser:getUser·p1.00          sample        17.695           ms/op
ClientSimple.listUser                       sample  2992  10.771 ± 0.233   ms/op
ClientSimple.listUser:listUser·p0.00        sample         3.117           ms/op
ClientSimple.listUser:listUser·p0.50        sample        10.109           ms/op
ClientSimple.listUser:listUser·p0.90        sample        13.774           ms/op
ClientSimple.listUser:listUser·p0.95        sample        15.408           ms/op
ClientSimple.listUser:listUser·p0.99        sample        37.168           ms/op
ClientSimple.listUser:listUser·p0.999       sample        41.551           ms/op
ClientSimple.listUser:listUser·p0.9999      sample        45.285           ms/op
ClientSimple.listUser:listUser·p1.00        sample        45.285           ms/op

Benchmark result is saved to 1723377997165.json
